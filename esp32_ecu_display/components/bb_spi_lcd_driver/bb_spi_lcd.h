...
void s3_alpha_blend_be(uint16_t *pFG, uint16_t *pBG, uint16_t *pDest, uint32_t c
ount, uint8_t alpha, const uint16_t *pMasks);
void s3_masked_tint_be(uint16_t *pDest, uint16_t *Src, uint16_t *pMask, uint16_t
 tintColor, uint32_t count, uint8_t alpha, const uint16_t *pMasks);
void s3_blur_be(uint16_t *pSrc, uint16_t *pDest, uint32_t count, uint32_t pitch,
 const uint32_t *pMasks);
void s3_byteswap(uint16_t *pSrc, uint16_t *pDest, int iPixelCount);
void s3_alphatrans_be(uint16_t *pFG, uint16_t *pBG, uint16_t *pDest, uint32_t co
unt, uint8_t alpha, uint16_t *transparent, const uint16_t *pMasks);
}
#endif // S3 SIMD
#endif // __has_include
#endif // ESP32
...
#include <SPI.h>
#include <Print.h>
#if defined( ARDUINO_M5Stick_C ) || defined (ARDUINO_M5STACK_Core2) || defined(A
RDUINO_M5STACK_CORES3)
#include <Wire.h>
#endif
#else
#define false 0
...
typedef struct tagSPILCD
{
   ...
   SPIClass *pSPI;
#else
   void *pSPI; // For ESP-IDF, this will be spi_device_handle_t
#endif
